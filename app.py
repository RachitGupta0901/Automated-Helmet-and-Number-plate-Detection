...
# Open the webcam for video capture (use 0 for the default camera)
...
output_folder = 'captured_images'  # Folder to save captured images
        ...
        if i in indexes:
            ...
            if classIds[i] == 0:  # bike
                ...
                if y_h > 0 and x_h > 0:
                    ...
                    # Check for no helmet detection
                    if c == 1:  # no-helmet
                        no_helmet_detected = True

    # Save the image when no helmet is detected along with the number plate
    if no_helmet_detected:
        image_counter += 1
        output_filename = os.path.join(output_folder, f'no_helmet_and_number_plate_{image_counter}.jpg')
        cv2.imwrite(output_filename, img)

    # writer.write(img)
    # cv2.imshow("Image", img)
    ret, buffer = cv2.imencode('.jpg', img)
    frame = buffer.to_bytes()
